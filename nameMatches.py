...
class PlayerMatchesBySite:
    ...
    def fuzzy_match(self, match_row):
        name_to_match = match_row['fotmob']
    ...
    def common_lastname(self, match_row):
    ...
    async def wiki_search(self, match_row):
    ...
    async def wiki_names(self, match_row):
    ...
    async def wiki_name_match(self, match_row):
    ...
    def add_team_name(self, match_row):
        match_row['team'] = self.team_name
        return match_row

    def match_stage_sync(self, func):
    ...
    def apply_match_stages_sync(self, match_funcs):
        for match_function in match_funcs:
            if self.match_rows:
                self.match_stage_sync(match_function)

    async def match_stage_async(self, func):
        current_match_rows = self.match_rows
    ...
    async def apply_match_stages_async(self, match_funcs):
        for match_function in match_funcs:
            if self.match_rows:
