...
The pipeline stages either set or modify class variables within a PipeBase instance.
"""
...
class SplitBySite(PipeBase):
    ...
    site_dfs_nodups -- site_dfs without duplicate names since initial join is based on processedName
...
class InitialSiteJoin(PipeBase):
    """Joins site dfs by processedName to account for links where same name exists across all sites

    Sets:
    player_match_df -- DataFrame to store player match/join results between site_dfs
    """
    def initial_join(self, sites: list[pd.DataFrame]) -> pd.DataFrame:
        join_df = sites[0]
        for site in sites[1:]:
...
class ClubNameMatches(PipeBase):
    """Based on InitialSiteJoin matches, store team name links across sites
    ...
    def apply_map_to_matchdf(self) -> None:
        cols = self.match_cols()
        for col in cols:
...
class SplitSitesByClub(PipeBase):
    ...
    players_by_club
    """
    def players_left_by_site(self, sites: list[pd.DataFrame]) -> list[pd.DataFrame]:
